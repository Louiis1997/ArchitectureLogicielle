...
import fr.esgi.cc2.kernel.CommandBus;
import fr.esgi.cc2.kernel.QueryBus;
import fr.esgi.cc2.use_cases.member.application.CreateMember;
import fr.esgi.cc2.use_cases.member.application.RetrieveMembers;
...
public class MemberController {
    ...
    public ResponseEntity<MembersResponse> getAll() {
        ...
        MembersResponse usersResponseResult = new MembersResponse(members.stream().map(member -> new MemberResponse(String.valueOf(member.getId().getValue()), member.getLastname(), member.getFirstname(),new EmailResponse(member.getEmail().getEmail()),member.getPassword(), member.getRole(), member.getCreditCard())).collect(Collectors.toList()));
